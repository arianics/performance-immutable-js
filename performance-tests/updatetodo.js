...
var test1 = function () {
  _.each(range, (index) => {
    ...
    todo.title = "New Title " + index;
    immutableTodos = immutableTodo.updateTodo(immutableTodos, todo);
  });
};
...
var test2 = function () {
  _.each(range, (index) => {
    ...
    todo.title = "New Title " + index;
    jsTodos = jsTodo.updateJsTodo(jsTodos, todo);
  });
};
...
var test3 = function () {
  _.each(range, (index) => {
    ...
    todo.title = "New Title " + index;
    es6Todos = es6Todo.updateEs6Todo(es6Todos, todo);
  });
};

pt.addPreTestScripts(preTest); // add test setup script
pt.addTestSnippet('immutable.updateTodo', test1); // add first test
pt.addTestSnippet('js.updateTodo', test2); // add second test
pt.addTestSnippet('es6.updateTodo', test3); // add second test
